...
def mas_KA(M_start):
    for i in speed_array:
        M_start = m_top / (1 - math.exp((-i)/I))
        m_KA = M_start - m_RB
        m_PG_array.append(m_KA)
...
# График зависимости массы КА от угла поворота в перигее
plt.plot(i_p_array, m_PG_array)
plt.grid()
plt.xlabel("Угол поворота в перигее, град.")
plt.ylabel("Масса КА, кг")
plt.show()
...
for j in range(80000, 400000, 60000):
    ...
    # Расчет массы батарей от времени перелета
    C = P * T
    massa_battery = C / C_u
...
# График зависимости времени перелета от высоты апогея переходной орбиты
plt.plot(time_array, battery_array)
plt.grid()
plt.xlabel("Время перелета, ч")
plt.ylabel("Масса батарей, кг")
plt.show()
